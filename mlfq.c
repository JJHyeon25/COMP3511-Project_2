...
void mlfq() {
	...
	int time = 0;
	//cumulative num of process that entered queue
	int num_process = 0;
	...
	queue_init(&q2);
	queue_init(&q3);
	//Measure the total burst time of all process
	for (int i = 0; i < MAX_NUM_PROCESS; ++i) {													
		time += process_table[i].burst_time;
	}
	//Make iterations for every single time unit
	for (int i = 0; i < time; ++i) {
		int index = 0;
		//If new process arrives
		if (i == process_table[num_process].arrival_time) {
			queue_enqueue(&q1, num_process);
			process_table[num_process].remain_time = tq0;
			++num_process;
		}
		//q1 not empty
		if (!queue_is_empty(&q1)) {
			--process_table[queue_peek(&q1)].burst_time;
			--process_table[queue_peek(&q1)].remain_time;
			//finish process
			if (process_table[queue_peek(&q1)].burst_time == 0) {
				index = queue_peek(&q1);
				queue_dequeue(&q1);
				gantt_chart_update(chart, &sz_chart, process_table[index].name, 1);
			//move to another queue
			} else if (process_table[queue_peek(&q1)].remain_time == 0) {
				index = queue_peek(&q1);
				queue_dequeue(&q1);
				queue_enqueue(&q2, index);
				process_table[index].remain_time = tq1;
				gantt_chart_update(chart, &sz_chart, process_table[index].name, 1);
			} else {
				gantt_chart_update(chart, &sz_chart, process_table[queue_peek(&q1)].name, 1);
			}
		//q2 not empty
		} else if (!queue_is_empty(&q2)) {
			--process_table[queue_peek(&q2)].burst_time;
			--process_table[queue_peek(&q2)].remain_time;
			//finished process
			if (process_table[queue_peek(&q2)].burst_time == 0) {
				index = queue_peek(&q2);
				queue_dequeue(&q2);
				gantt_chart_update(chart, &sz_chart, process_table[index].name, 1);
			//move to another queue
			} else if (process_table[queue_peek(&q2)].remain_time == 0) {
				index = queue_peek(&q2);
				queue_dequeue(&q2);
				queue_enqueue(&q3, index);
				gantt_chart_update(chart, &sz_chart, process_table[index].name, 1);
			} else {
				gantt_chart_update(chart, &sz_chart, process_table[queue_peek(&q2)].name, 1);
			}
		//for q3
		} else {
			--process_table[queue_peek(&q3)].burst_time;
			//finished process
			if (process_table[queue_peek(&q3)].burst_time == 0) {
				index = queue_peek(&q3);
				queue_dequeue(&q3);
				gantt_chart_update(chart, &sz_chart, process_table[index].name, 1);
			} else {
				gantt_chart_update(chart, &sz_chart, process_table[queue_peek(&q3)].name, 1);
			}
		}
	}
